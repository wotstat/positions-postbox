...
// Source: https://github.com/alexstep/moy-nalog/tree/main
...
export class NalogAPI {
  ...
  private authPromise: Promise<any> | null
  ...
  async getToken(): Promise<any> {
    ...
    }).then(r => r.json()).catch(console.error) as any
   ...
   */
  async call(endpoint: string, payload: any = null, method: string = 'GET'): Promise<any> {
  ...
  /**
   * Добавления "прихода"
   * @param  {date} date=now - время поступления денег
  ...
  async addIncome({ date = new Date(), name, quantity = 1, amount }: { date?: Date, name: string, quantity?: number, amount: number }) {
    ...
    const result = {
      id: response.approvedReceiptUuid as string,
      approvedReceiptUuid: response.approvedReceiptUuid as string,
  ...
  dateToLocalISO(date: Date = new Date(), offsetMinutes = PLUS_3): string {
    ...
    return shifted.toISOString().replace("Z", offset).replace(/\.\d{3}/, "");
  }
}
